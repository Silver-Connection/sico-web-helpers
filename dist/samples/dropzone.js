$(document).ready(function () {
    ...
    var model = { Images: list };
    var vueModel = new sico.Transaction(model);
    var vueImages = new sico.VueHelper({
        data: vueModel.$vue(),
        delete: "/api/image/",
        el: "#tab-dropzone",
        methods: {
    ...
    sico.DropzoneHelper.$image(function (respond) {
        vueImages.vue.$data.Data.Images.push(respond.Data);
    });
    function imageDelete(id) {
        var index = 0;
        var match = vueImages.$find("Data.Images", function (el, i) {
            index = i;
            return el.Id === id;
        });
